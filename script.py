...
HOUR_ON,MIN_ON=17,15#Turn Light ON at 17:15 (so can't be negative
...
def loop():
    ...
    if ((morning) and (now.hour==MORN_OFF) and (now.minute==MORNMIN_OFF) and (now.second==0)):
            allOff(numberLights)            
    ...
    if ((now.hour == HOUR_ON) and (now.minute == MIN_ON) and (now.second <=5)) and (AUTOMAN=="AUTO"):
        allOn(numberLights)
        
        
    # toggle light OFF
    if ((now.hour == HOUR_OFF) and (now.minute == MIN_OFF) and (now.second <=5))and (AUTOMAN=="AUTO"):
        allOff(numberLights)
